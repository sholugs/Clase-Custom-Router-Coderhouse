...
handlePolicies = policies => (req, res, next) => {
    ...
    if (policies.length > 0) {
        ...
        if (!policies.includes(user.role.toUpperCase())) {
            return res.sendNoAuthorizatedError('Not authorized!')
        }

        req.user = user
    } else {
        // Si no hay políticas especificadas, solo verificamos la presencia del token
        const authHeaders = req.headers.authorization

        if (!authHeaders) {
            return res.sendNoAuthenticatedError('No authenticated!')
        }
        ...
        try {
            ...
            return res.sendNoAuthenticatedError('Invalid token!')
        }
    }
    next()
}
